module.exports=[182087,e=>{"use strict";var i=e.i(698043),t=e.i(129508),a=e.i(528257);let r=new class{transporter=null;isProcessing=!1;lastConfig="";getTransporter(){let e=(0,a.getLiveEnv)("SMTP_HOST","smtp.turkticaret.net"),i=Number((0,a.getLiveEnv)("SMTP_PORT","587"))||587,r=(0,a.getLiveEnv)("SMTP_USER",""),o=(0,a.getLiveEnv)("SMTP_PASS",""),l="true"===(0,a.getLiveEnv)("SMTP_SECURE","false"),n=`${e}-${i}-${r}-${o}-${l}`;if(!this.transporter||this.lastConfig!==n){if(this.transporter)try{this.transporter.close()}catch(e){console.error("[EmailNotificationService] Error closing old transporter:",e)}this.transporter=t.default.createTransport({pool:!0,host:e,port:i,secure:l,auth:{user:r,pass:o},connectionTimeout:15e3,greetingTimeout:15e3,socketTimeout:2e4,maxConnections:5,maxMessages:100,rateLimit:10,tls:{rejectUnauthorized:!1}}),this.lastConfig=n,console.log("[EmailNotificationService] Nodemailer transporter initialized dynamically.")}return this.transporter}async seedDefaultTemplate(e){let t={welcome:{name:"Hoş Geldiniz (B2C)",subject:"Pekefe Ailesine Hoş Geldiniz! 🌿",variables:"kullanici_adi",bodyHtml:`
        ...
        `},order_status_updated:{name:"Sipariş Durum Güncellemesi",subject:"Siparişinizin Durumu Güncellendi: {{siparis_durumu}} — Sipariş No: {{siparis_no}} 📦",variables:"kullanici_adi,siparis_no,siparis_durumu,siparis_tutari,kargo_sirketi,takip_no,detay_linki,tarih",bodyHtml:`
          <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 620px; margin: 0 auto; border: 1px solid #f1f5f9; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.06); background-color: #ffffff;">
            <!-- Header -->
            <div style="background: linear-gradient(135deg, #6b1d2f 0%, #3b0a18 100%); padding: 36px 32px; text-align: center;">
              <h2 style="color: #ffffff; margin: 0; font-size: 28px; font-weight: 900; letter-spacing: 0.05em;">PEKEFE</h2>
        ...
        `},newsletter_welcome:{name:"Bülten Aboneliği Hoş Geldiniz",subject:"Pekefe Bülten Kulübüne Hoş Geldiniz! 🌿",variables:"email,brandName,siteUrl",bodyHtml:`
          <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #f1f5f9; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.06); background-color: #ffffff;">
            <div style="background: linear-gradient(135deg, #6b1d2f 0%, #3b0a18 100%); padding: 36px 32px; text-align: center;">
        ...
        `},ekstre:{name:"Cari Hesap Ekstre Gönderimi",subject:"Cari Hesap Ekstresi — {{tarih}} | Pekefe",variables:"kullanici_adi,cari_kod,bakiye,bakiye_durumu,bakiye_rengi,bakiye_etiketi,tarih,yetkili,telefon,vade_gun,adres",bodyHtml:`
          <!DOCTYPE html>
          <html lang="tr">
        ...
        `},admin_new_order:{name:"Yeni Sipariş Bildirimi (Yönetici)",subject:"🚨 Yeni Sipariş Alındı! — Sipariş No: {{siparis_no}}",variables:"kullanici_adi,siparis_no,siparis_tutari,odeme_yontemi,detay_linki,tarih,siparis_icerik",bodyHtml:`
          <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #f1f5f9; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.06); background-color: #ffffff;">
            <div style="background: linear-gradient(135deg, #6b1d2f 0%, #3b0a18 100%); padding: 36px 32px; text-align: center;">
        ...
        `}}[e.toLowerCase().trim()];if(!t)throw Error(`Default template not found for event: ${e}`);let a=await i.prisma.emailTemplate.upsert({where:{eventType:e.toLowerCase().trim()},update:{name:t.name,subject:t.subject,bodyHtml:t.bodyHtml,variables:t.variables,status:"ACTIVE"},create:{eventType:e.toLowerCase().trim(),name:t.name,subject:t.subject,bodyHtml:t.bodyHtml,variables:t.variables,status:"ACTIVE"}});return console.log(`[EmailNotificationService] Upserted default template for eventType: ${e}`),a}async queueEmail(e,t,a,r=3){let o=t.toLowerCase().trim(),l=await i.prisma.emailTemplate.findUnique({where:{eventType:o}});if(!l)try{l=await this.seedDefaultTemplate(o)}catch(e){console.error(`[EmailNotificationService] Failed to seed template for ${t}:`,e)}if(!l)throw Error(`Email template for event ${t} could not be resolved.`);if("ACTIVE"!==l.status)throw Error(`Email template for event ${t} is currently inactive.`);let n=this.compileTemplate(l.subject,a),d=this.compileTemplate(l.bodyHtml,a),p=await i.prisma.emailLog.create({data:{recipient:e,subject:n,bodyHtml:d,eventType:o,status:"PENDING",retryCount:0}});return await this.processQueue(),p.id}compileTemplate(e,i){let t=e;return Object.entries(i).forEach(([e,i])=>{let a=RegExp(`{{\\s*${e}\\s*}}`,"g");t=t.replace(a,null!=i?String(i):"")}),t=t.replace(/{{\s*\w+\s*}}/g,"")}async processQueue(){if(!this.isProcessing){this.isProcessing=!0;try{for(let e of(await i.prisma.emailLog.findMany({where:{status:"PENDING"},take:20,orderBy:{createdAt:"asc"}})))try{let t=this.getTransporter(),r=(0,a.getLiveEnv)("SMTP_USER","info@pekefe.com"),o=(0,a.getLiveEnv)("SMTP_FROM_NAME","Pekefe");await t.sendMail({from:`"${o}" <${r}>`,to:e.recipient,subject:e.subject,html:e.bodyHtml}),await i.prisma.emailLog.update({where:{id:e.id},data:{status:"SENT"}})}catch(a){let t=e.retryCount+1;await i.prisma.emailLog.update({where:{id:e.id},data:{retryCount:t,status:t>=3?"FAILED":"PENDING",errorMessage:a?.message||"Send failed"}})}}catch(e){console.error("[EmailNotificationService] Error processing email queue:",e)}finally{this.isProcessing=!1}}}};e.s(["emailNotificationService",0,r])}];

//# sourceMappingURL=src_lib_email-notification-service_ts_1iz_3g_._.js.map