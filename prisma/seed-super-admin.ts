/**
 * Super Admin + Feature Modules Seed Script
 * Çalıştırmak için: npx ts-node prisma/seed-super-admin.ts
 */
...
async function main() {
  ...
  // ── 2. SUPER_ADMIN kullanıcısını oluştur / güncelle
  ...
  // ── 3. Mevcut ADMIN kullanıcılarını listele ve hepsini onaylı yap
  console.log("\n👥 Mevcut kullanıcılar:");
  ...
  for (const u of users) {
    ...
    console.log(`  ${marker} ${(u.role || "").padEnd(15)} | ${(u.email || "").padEnd(40)} | ${u.name || ""} | approved:${u.isApproved}`);
  }

  // ── 4. Tüm Company'ler için tüm feature modüllerini etkinleştir
  const companies = await (prisma as any).company.findMany({ select: { id: true, name: true } });
  const features = await (prisma as any).featureModule.findMany({ select: { id: true, key: true } });

  if (companies.length > 0) {
    console.log(`\n🏢 ${companies.length} şirket için feature izinleri ayarlanıyor...`);
    for (const company of companies) {
      for (const feature of features) {
        await (prisma as any).companyPermission.upsert({
          where: {
            companyId_featureModuleId: {
              companyId: company.id,
              featureModuleId: feature.id,
            },
          },
    ...
    console.log("\n⚠️  Kayıtlı şirket bulunamadı. Şirket oluşturulduktan sonra izinler otomatik verilecek.");
  }

  console.log("\n✨ Seed tamamlandı!");
  console.log("─────────────────────────────────");
  console.log("Giriş bilgileri:");
  console.log("  E-posta : superadmin@sistem.local");
