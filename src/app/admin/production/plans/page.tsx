    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-red-50 border border-red-100 rounded-2xl max-w-xl mx-auto my-12 text-center">
        ...
        <p className="text-xs text-slate-500 mt-2">
          {result.error || "Plan verileri yüklenirken bir hata oluştu."}
        </p>
      </div>
    );
  }
      ...
      <Suspense
        fallback={
          <div className="flex flex-col items-center justify-center py-24 space-y-4">
            ...
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Planlar Yükleniyor...</p>
          </div>
        }
      >
        <PlansClient initialData={result.data} />
      </Suspense>
    </div>
  );
}
