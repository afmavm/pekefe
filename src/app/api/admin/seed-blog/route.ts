...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `${results.length} adet blog yazısı başarıyla veritabanına eklendi / güncellendi.`,
      posts: results,
    });
  } catch (error) {
    console.error("Blog seed error:", error);
    return NextResponse.json({ error: "Veritabanına yazılırken hata oluştu." }, { status: 500 });
  }
}
